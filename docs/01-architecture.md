...
| Root `/`       | 301 → `/en/` (`_redirects` per host + HTML fallback page, docs/08 §7 Q1 — option A)                                   | The gate was removed 2026-09-21: a language chooser on `/` split the brand-query equity between three documents; the EN home now owns it, RU stays one click away (`/ru/`, header switch). `x-default` follows the redirect. |
...
| SEO gate       | `npm run audit:seo` (`scripts/seo-audit.mjs`) over `dist`                                                              | 152 страниц (75 × 2 локали + root-redirect + 404) нельзя проверить руками; the audit is the contract (see 02).                                                                                                                                              |
