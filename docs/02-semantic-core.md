...
3. `img` with a missing, empty or filename-like `alt` (the empty-string exemption is gone — 136
   images quietly shipped `alt=""` behind it) or without an explicit `loading="eager|lazy"`
...
`noindex` ones, JSON-LD that parses, alt text on every `<img>`, explicit loading strategy, heading
levels, ru/en parity per path, per-host sitemap coverage, duplicate titles, duplicate body text
(md5 of visible copy), relative and cross-host links, `/files/` assets, dev URLs, CJK characters. Run
it after `npm run build:hosts`; the point is that a rule missing from `seo-audit.mjs` still surfaces.
