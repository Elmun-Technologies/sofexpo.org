...
| `QuizForm.astro`                                         | `title?, subtitle?, shows?, areas?, note?, compact?, id`                                                                    | 4-step lead quiz (rush-agency.ru pattern): tap-only questions auto-advance, contact fields last, success summary; same endpoint/localStorage contract as `LeadForm`                     |
| `StandQuizModal.astro`                                   | `locale`                                                                                                                    | global `<dialog>` around `QuizForm`; opened by any `[data-quiz-open]` CTA (header "Book a stand"), href fallback without JS                                                            |
...
| `quiz`      | `shows?, areas?, note?`                                         | `QuizForm` in-page (4 steps, narrow column)                                |
...
- `og/default.jpg` (1 200×630, 80 KB), the favicon PNGs and each branded host's own
  `public/brand/<id>/{favicon.png,apple-touch-icon.png,og.jpg}` are generated by
  `node scripts/build-assets.mjs`: the centre card from `public/images/venue-exterior.jpg` +
  `public/favicon.svg`, the brand cards from `src/data/brand-map.json` + `public/brand/<id>/favicon.svg`,
  all with sharp — none of them is edited by hand, and the card carries no dates on purpose (dates
  live in `src/data/events.ts`; a second copy of a fact is a fact that can drift).
...
Тон знака задаётся не «на глаз», а контрастом: на тёмном поле квадрат белый, ромб охрой
(`--gold-soft`), на бумаге квадрат синий `--brand-mark`, ромб `--gold` — потому что `#2c7fbb` на
тёмном даёт 2.52, а `#c3a06a` на бумаге 2.34, т.е. половина знака просто исчезла бы.
...
**Пять хостов, пять палитр (2026-09-22).** В `brand-map.json` теперь все 5 выставок: у каждой
своя палитра (14 токенов + замеры контраста в поле `contrast`) и живой текстовый wordmark
(`Header`/`Footer`: имя выставки + год). Граница правила «не выдумывать факт» не сдвинута —
палитра и название это собственный дизайн-слой сайта, а **знак** остаётся фактом:
`mark: "star"` = eight-pointed звезда с файла логотипа организатора (FOODERA), `mark: null` =
знака нет: в header/footer только wordmark, а на соц-карте — знак центра (зал: оболочка +
модули), перепечатанный в палитре хоста. Когда файл бейджа организатора приедет — одна строка
`mark: "star"` + свой `favicon.svg`, и хост получит свою звезду.

Отдельно: `public/brand/{foodera,buildpro,agropro,world-edu,ecom-retail}/`
(favicon.svg/png, apple-touch, og.jpg) попадает в сборку каждого хоста — это 5 маленьких
каталогов, и ссылается на них только брендированный хост; вырезать их пер-хостовой сборкой
