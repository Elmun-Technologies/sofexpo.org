...
for (const { host } of ordered) {
  const label = eventOfHost(host)
    ? `${host} (${eventOfHost(host)})`
    : `${host} (centre)`;
  ...
  execFileSync("node", ["scripts/prune-sitemap.mjs", "dist"], {
    env,
    stdio: "inherit",
  });
  execFileSync(
  ...
  report.push({
    host,
    dir: target,
    redirects: redirects ? redirects.trim().split("\n").length : 0,
  });
}
...
if (!flag("no-audit")) {
  for (const { host, dir } of report) {
    ...
    execFileSync(
      "node",
      ["scripts/seo-audit.mjs", dir, "--host", host, "--peers", out],
      {
        env: {
          ...process.env,
          SITE: `https://${host}/`,
  ...
  execFileSync(
    "node",
    ["scripts/check-hosts.mjs", "--dir", out, "--mode", mode],
    { stdio: "inherit" },
  );
}
