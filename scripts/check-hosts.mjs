...
function fileFor(root, href) {
  const rel = href.replace(/^https?:\/\/[^/]+/, '').replace(/^\/+/, '').replace(/\/+$/, '');
  return rel ? join(root, rel, 'index.html') : join(root, 'index.html');
}

const builtBy = new Map(); // host -> Set of host-local paths
const hosts = readdirSync(DIR).filter((name) => statSync(join(DIR, name)).isDirectory());
if (!hosts.length) {
  console.error(`check-hosts: nothing found in ${DIR}/ — run \`npm run build:hosts -- --mode ${mode}\` first`);
  process.exit(2);
}
...
for (const host of hosts) {
  ...
  for (const file of htmlFiles(join(DIR, host))) {
    const rel = ('/' + file.slice(join(DIR, host).length).replace(/\\/g, '/').replace(/index\.html$/, '').replace(/^\/+/, '')).replace(/\/+/g, '/');
  ...
  for (const file of htmlFiles(root)) {
    ...
    for (const m of html.matchAll(/<a\b[^>]*?href="([^"#]*?)(?:[#?][^"]*)?"/g)) {
      ...
      if (/^https?:\/\//.test(href)) {
        ...
        if (!u.hostname.endsWith('sofexpo.org') && u.hostname !== map.root) continue;
        if (!known.has(u.hostname)) {
          warn(`${shown}: link to an unregistered hostname ${u.hostname}`);
          continue;
        }
        if (u.hostname === host) {
          warn(`${shown}: absolute link to its own host — should be relative: ${href}`);
          continue;
        }
        checkedAbsolute++;
        ...
        const want = u.pathname.endsWith('/') || isFile ? u.pathname : `${u.pathname}/`;
        const peerRoot = join(DIR, u.hostname);
        const ok = isFile
          ? existsSync(join(peerRoot, u.pathname.replace(/^\/+/, ''))) || existsSync(join(DIR, map.root, u.pathname.replace(/^\/+/, '')))
          : target.has(want) || existsSync(fileFor(peerRoot, u.pathname));
        if (!ok) warn(`${shown}: link to ${u.hostname}${want} — no such page on that host`);
        ...
        const ok = isFile ? existsSync(join(root, href.replace(/^\/+/, ''))) : target.has(want) || existsSync(fileFor(root, href));
        if (!ok) warn(`${shown}: relative link ${want} is not built on ${host} (it lives on another host)`);
      }
    }
    ...
    const isStub = /name="robots" content="noindex/.test(html) && /http-equiv="refresh"/.test(html);
    if (!isStub) {
      for (const m of html.matchAll(/<link rel="alternate" hreflang="[a-z-]+" href="(https?:\/\/[^/]+)/g)) {
        if (m[1] !== `https://${host}`) warn(`${shown}: hreflang points at ${m[1]} instead of ${host}`);
      }
    }

    /* 3: one path, one owner */
    const self = ('/' + file.slice(root.length).replace(/index\.html$/, '').replace(/^\/+/, '')).replace(/\/+/g, '/');
...
for (const [path, owners] of ownerConflicts) {
  ...
  if (owners.length > 1 && sameContent.size === owners.length)
    warn(`path ${path} is built on ${owners.length} hosts (${owners.join(', ')}) — duplicated across hostnames`);
}
...
if (mode === 'subdomain') {
  ...
  if (!expected.length) {
    console.log('check-hosts: no cluster was moved (alias mode) — nothing to redirect');
  } else if (!existsSync(file)) {
    warn(`root host has no _redirects, but ${expected.length} cluster URL(s) moved to their own hosts`);
  } else {
    const txt = readFileSync(file, 'utf8');
    const missing = expected.filter((r) => !txt.includes(`${r.from} ${r.to}`));
    for (const r of missing) warn(`_redirects is missing: ${r.from} -> ${r.to}`);
  }
}
...
for (const host of skipped)
  console.log(`note: peer ${host} is not in ${DIR}/ — links to it were not verified (build it with npm run build:hosts)`);
...
console.log('✓ hosts agree: every internal link resolves on the host that owns it');
