...
/** Post paths published on a show hostname (generated by scripts/gen-editorial-map.mjs). */
function loadEditorial() {
  const tries = [
    `${process.cwd()}/src/data/editorial-owners.json`,
    fileURLToPath(
      new URL("../src/data/editorial-owners.json", import.meta.url),
    ),
    fileURLToPath(
      new URL("../../../src/data/editorial-owners.json", import.meta.url),
    ),
  ...
  for (const file of tries) {
    try {
      return JSON.parse(readFileSync(file, "utf8")).map ?? {};
    } catch {
      /* next */
    }
  }
...
export const editorialOwners = loadEditorial();
...
export function ownerOf(
  path,
  mode = "alias",
  currentHost = ROOT_HOST,
  editorial = editorialOwners,
) {
  const clean = stripLocale(path);
  if (clean === "/") return { host: currentHost, path: "/" };
  if (mode === "subdomain" && editorial) {
    const owned = editorial[clean];
    if (owned?.host) return { host: owned.host, path: clean };
  }
  if (mode === "subdomain") {
...
export function hrefFor(
  locale,
  path,
  { mode = "alias", currentHost = ROOT_HOST } = {},
) {
