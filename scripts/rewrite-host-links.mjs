...
if (mode !== 'subdomain') {
  console.log('host-links: mode is not "subdomain" — every path lives on this host, nothing to rewrite');
  process.exit(0);
}
...
for (const file of files) {
  const rel = '/' + file.slice(dist.length).replace(/\\/g, '/').replace(/index\.html$/, '').replace(/^\/+/, '');
...
for (const file of files) {
  ...
  const next = html.replace(RE, (whole, locale, rest = '/') => {
    ...
    if (owner.host === host) return whole; // same host: relative is right
    const href = `https://${owner.host}${joinLocale(locale, owner.path)}`;
    changed++;
    return `href="${href}"`;
  });
...
console.log(`host-links: ${files.length} file(s), ${scanned} locale link(s), ${changed} rewritten to absolute`);
