...
for (const file of files) {
  ...
  if (title) {
    if (title.length > (isPost ? 96 : 78)) warn(url, `title too long (${title.length} chars)`);
  ...
  if (desc && (desc.length < 60 || desc.length > (isPost ? 210 : 185))) warn(url, `description length ${desc.length}`);
  ...
  if (!noindex) {
    ...
    else if (!canonical.startsWith('https://')) warn(url, `canonical is not absolute: ${canonical}`);
  }
  ...
  if (!noindex) {
    ...
    if (!alts.some((a) => a.lang === 'x-default')) warn(url, 'missing x-default alternate');
    for (const lang of ['ru', 'en']) {
      ...
      if (!allUrls.has(hrefPath) && hrefPath !== want) warn(url, `hreflang ${lang} points to ${hrefPath} (not built)`);
    }
  }
  ...
  for (const m of html.matchAll(/<a\b[^>]*href="([^"#?]+)(?:[#?][^"]*)?"/g)) {
    ...
    const ok = isFile ? existsSync(join(ROOT, target.replace(/^\//, ''))) : allUrls.has(target);
    if (!ok) warn(url, `broken internal link → ${target}`);
    else if (target !== url && allUrls.has(target)) inbound.set(target, (inbound.get(target) ?? 0) + 1);
  }
  ...
  for (const m of html.matchAll(/<img\b[^>]*>/g)) {
    ...
    else if (!/alt="[^"]{8,}"/.test(tag) && !/alt=""/.test(tag)) warn(url, `img with thin alt: ${tag.slice(0, 90)}`);
    if (!/loading=/.test(tag) && !/fetchpriority="high"/.test(tag)) warn(url, 'img without loading strategy hint');
  }
}

// duplicate metadata across indexable pages
for (const [map, label] of [[titleByUrl, 'title'], [descriptions, 'description']]) {
  const seen = new Map();
...
for (const url of allUrls) {
  ...
  const html = readFileSync(join(ROOT, url.replace(/^\//, ''), url.endsWith('/') ? 'index.html' : ''), 'utf8');
  if (!url.startsWith('/ru/') && !url.startsWith('/en/')) continue;
  if ((html.match(/<meta name="robots" content="([^"]+)"/) ?? [])[1]?.includes('noindex')) continue;
  if ((inbound.get(url) ?? 0) === 0) warn(url, 'orphan page (no internal links)');
}
...
for (const url of allUrls) {
  ...
  if (!allUrls.has(other)) warn(url, `no sibling in the other locale: ${other}`);
}

const indexHtml = existsSync(join(ROOT, 'index.html'));
if (!indexHtml) warn('/', 'missing root language gate');
for (const asset of ['robots.txt', 'sitemap-index.xml', 'sitemap-0.xml', 'llms.txt', 'site.webmanifest', 'favicon.svg', 'og/default.jpg']) {
...
console.log(`pages: ${counts.pages} (ru ${counts.ru} / en ${counts.en}) · distinct internal targets: ${counts.links}`);
...
for (const p of problems) {
  const key = p.msg.replace(/\(.*?\)/g, '').replace(/: .*/, '').replace(/^\w+ \d+ chars$/, (s) => s);
  byKind.set(key, (byKind.get(key) ?? 0) + 1);
}
console.log(`✗ ${problems.length} problem(s):\n`);
for (const [kind, n] of [...byKind.entries()].sort((a, b) => b[1] - a[1]).slice(0, 14)) console.log(`  ${String(n).padStart(4)}  ${kind}`);
console.log('\nfirst 40 details:');
