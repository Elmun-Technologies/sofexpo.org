...
export async function GET(context: APIContext) {
  ...
  const items = [
    ...posts
      ...
      .map((e) => ({
        ...
        pubDate: e.data.date,
        link: abs(`/${locale}/news/${e.id.split('/').slice(1).join('/')}/`),
    ...
    ...articles
      ...
      .map((e) => ({
        ...
        description: e.data.description,
        pubDate: e.data.date,
        link: abs(`/${locale}/articles/${e.id.split('/').slice(1).join('/')}/`),
  ...
  return rss({
    title: ru ? 'SOF EXPO Samarkand — новости и выставки' : 'SOF EXPO Samarkand — news and exhibitions',
    description: ru
